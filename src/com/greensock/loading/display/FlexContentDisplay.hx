...
class FlexContentDisplay extends UIComponent {
	...
	public function dispose(unloadLoader:Bool = true, disposeLoader:Bool = true):Void {
		if (this.parent != null) {
			if (this.parent.exists("removeElement")) {
				(try cast(this.parent, Dynamic) catch(e:Dynamic) null).removeElement(this);
			}
			else {
	...
	private function set_loader(value:LoaderItem):LoaderItem {
		...
		if (Std.is(_loader.vars.container, DisplayObjectContainer)) {
			if (_loader.vars.container.exists("addElement")) {
				(try cast(_loader.vars.container, Dynamic) catch(e:Dynamic) null).addElement(this);
			}
			else {
